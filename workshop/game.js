...
for (let i = 0; i < 5; i++) gameState.entities.push(new Robot());
...
canvas.addEventListener('click', function(event) {
  ...
  if (element.offsetParent) {
    ...
    let offsetX2 = (x / width) * canvas.width;
    let offsetY2 = (y / width) * canvas.width;
    
    gameState.entities.push(new Food(offsetX2, offsetY2 ))
});
let then = Date.now();
const gameLoop = () => {
  try {
    ...
    // ctx.fillText(
    //   'gameState.keysDown: ' + Object.keys(gameState.keysDown),
    //   0,
    //   108
    // );
    ...
    gameState.entities = gameState.entities.filter(entity => entity.energy > 0);
